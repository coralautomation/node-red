...
module.exports = function(RED) {
	...
	function checkLastEvent(id)
	    {
		var that =id; 
	        console.log(id);
		var soc = node._clients[id];
		if(soc)
		{
		   ...
		   if(soc.lastEvent)
		   {
			...
			var seconds = (timenow - soc.lastEvent)/1000;
			console.log(datenow.toLocaleTimeString() + " : last Event found at :" + seconds);
		 	soc.lastEvent = undefined;
		  	setTimeout(function(){
		  		checkLastEvent(that);
		   		},60*1000);
		   } 
        ...
        function handleConnection(/*socket*/socket) {
			...
			while(id in node._clients)
			{
				id = (1+Math.random()*4294967295).toString(16);
			}
            if (node.isServer) {
                node._clients[id] = socket;
                socket.lastEvent = Date.now();
		checkLastEvent(id);
		    node.emit('opened',{count:Object.keys(node._clients).length,id:id});
				console.log("Opened " + id);
				var data = {"Connected":true, "clientId" : id};
				node.handleEvent(id,socket,'connect',data);
		...
		node.handleEvent(id,socket,'message',data,flags);
            ...
            socket.on('error', function(err) {
				...
				var data = {"Error":err};
				node.handleEvent(id,socket,'error',data);
